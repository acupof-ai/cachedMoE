...
| # | v0.1 的问题 | v0.2 |
|---|---|---|
| 1 | `hq8` 的 32 行约束让 M=1 损失 27%（6.3 ms/token） | **解决**（§9）。`HQuant = 3` 把量化搬进第三个 dispatch，M=1 回到 `L32 R1`，代价从 +23…29% 降到 **−0.5% / +1.7% / +3.9% / +4.2%（四轮，中位数约 +3%）**，即 6.3 → **约 1.0 ms/token** |
| 2 | x 在 kernel 外量化成 int8 能不能到 187 GB/s | **实现了，A 稳定快 15–30%，但两条都不达标**（§10）。A+B 在 M=6 上仍只有 63–67% 上限，端到端收益 0–7% 且不稳定，M=1 是负收益；精度在第二个 golden 上 **8.9e-3**，超 §12 的 5e-3。`XMode = 6` **默认关闭** |
...
v0.2 那一轮的 M 扫描（不量化的最优变体，上限 211.1–217.2 GB/s，
`bench/results/kernel_p2b_moe.csv`）——**和 §3.5 是同一个形状、同一个量级，
v0.2 没有让任何 M 变慢**：

| M | 最优变体 | A+B GB/s | %上限 | ms/pair | **ms/token** |
|---|---|---|---|---|---|
| 1 | `L32 R1 xglob` | 220.7 | 102% | 0.596 | **0.5962** |
| 2 | `L32 R1 xgf16` | 207.2 | 95% | 0.635 | **0.3175** |
| 3 | `L32 R1 xglob` | 175.3 | 81% | 0.751 | **0.2503** |
| 4 | `L16 R2 xldsi8` | 164.7 | 76% | 0.799 | **0.1998** |
| 5 | `L16 R2 xgf16/ldsi8` | 154.3 | 71% | 0.853 | **0.1706** |
| 6 | `L16 R2 xgf16/ldsi8` | 146.7 | 67% | 0.897 | **0.1495** |

开着 `HQuant = 3`（也就是 design §7.9 v0.6 要求的 `h` 量化，
这才是产线上真正要跑的东西）的同一张表在 §9.4。
...
| 1 | `L16 R2 xgf16/ldsi8` | 0.780 | 0.792 | +1.5% | 0.792 |
| 2 | `L32 R1 xglob` | 0.670 | 0.778 | +16.1% | 0.389 |
| 2 | `L16 R2 xgf16/ldsi8` | 0.841 | **0.851** | **+1.2%** | 0.425 |
| 3 | `L32 R1 xglob` | 0.777 | 0.914 | +17.6% | 0.305 |
...
**在每个 M 真正会用的形状上，`hqP` 的代价是 0.0% … +5.4%**，
M ∈ {2,3,4,5} 在 ±2.5% 以内（也就是 §2 的漂移里），两端的 M=1 和 M=6 各差 4–5%。
M=1 一共量了四轮：**−0.5% / +2.2% / +3.9% / +4.2%**
（`--only "h fp8"` 两轮、全 sweep 两轮），中位数约 +3%。
对比 v0.1 的 `hq8` 在 M=1 上的 +23…27%：
**decode 的量化税从 6.3 ms/token 降到约 1.0 ms/token。**

M=1 那 +4.2% 拆得出来：同一行的 `ms_a`（0.401 对 0.401）和 `ms_b`
（0.226 对 0.223）与不量化的**一模一样**，差额整个 0.025 ms 出在
A+B 一起跑的时候——也就是**多出来的那个 dispatch 与它的全局 barrier**，
不是任何一段计算。要再往下走只能减 barrier，那是 `gpu/vulkan/cmdbuf.cpp`
的事，不在这一轮的范围里。

`L32 R1` 在 M ≥ 2 上那 16–18% 不是 `hqP` 的问题：是 **dispatch B 在 R=1 上
给 M 个列做 fp8 反量化**（M=2 时 B 从 180.7 掉到 123.6 GB/s），
而 `L32 R1` 在 M ≥ 2 上本来就不是赢家。

### 9.4.1 和另外两种实现比（同一节，全 sweep 的那一轮）

| 变体 | A GB/s | B GB/s | A+B GB/s | ms/pair | 对同节对照 |
|---|---|---|---|---|---|
| M=1 `L32 R1 xglob`（不量化，对照） | 218.9 | 196.8 | 219.3 | **0.600** | — |
| M=1 `L32 R1 xglob hqP` | 219.5 | 194.1 | 215.6 | **0.610** | **+1.7%** |
| M=1 `L16 R2 xglob hq8` | — | — | 170.0 | 0.774 | +29.0% |
| M=1 `L32 R4 xglob hq8` | 177.6 | 157.9 | 170.4 | 0.772 | +28.7% |
| M=1 `L32 R1 xglob hqB` | 222.8 | 84.3 | 150.7 | 0.873 | +45.5% |
| M=6 `L16 R2 xgf16/ldsi8`（不量化，对照） | 141.3 | 134.6 | 146.7 | **0.897** | — |
| M=6 `L16 R2 xgf16/ldsi8 hqP` | 139.3 | 118.6 | 139.1 | **0.946** | **+5.5%** |
| M=6 `L16 R2 xgf16/ldsi8 hq8` | 136.3 | 118.7 | 135.1 | 0.974 | +8.6% |
| M=6 `L32 R1 xgf16 hqB` | 147.4 | 17.6 | 42.1 | 3.123 | +248% |

（`hqP` 在这一节里量到 0.610，在 §9.4 的成对小节里量到 0.625；
两个数在同一轮的不同小节，§2 的 8% 节间漂移解释得了这 2.5%。）

**M=6 上 `hqP` 比 `hq8` 快 3%**，M=1 上快 27%，没有一个 M 更慢：
`HQuant = 2` 现在没有任何理由再用。
`hqB` 在 `L32 R1` 上 dispatch B 只有 17.6–84.3 GB/s——它要为**每个权重行**
重算同一个块 amax，R=1 时一个都摊不掉——而
`runtime/moe_bridge.h` 现在的默认正是 `h_quant = 1`（§12）。
...
同一节内对照（`int8 x` 那一节带着自己的 fp16 对照），`bench/results/kernel_p2b_moe.csv`：

| M | 最优 fp16 对照 ms/token | 最优 int8-x ms/token | 变化 |
|---|---|---|---|
| 1 | `L32 R1 xglob` **0.597** | `L16 R4 prei8` 0.692 | +15.9% |
| 2 | `L32 R1 xglob` 0.340 | `L32 R1 prei8` **0.318** | −6.5% |
| 3 | `L16 R2 xgf16/ldsi8` 0.257 | `L32 R1 prei8` **0.251** | −2.3% |
| 4 | `L16 R2 xgf16/ldsi8` 0.216 | `L16 R2 prei8` **0.214** | −0.9% |
| 5 | `L16 R2 xgf16/ldsi8` 0.184 | `L16 R2 prei8` **0.184** | 0.0% |
| 6 | `L16 R2 xgf16/ldsi8` 0.161 | `L16 R2 prei8` **0.160** | −0.6% |

前两轮（`--only "int8 x"` 与上一次全 sweep）在 M=6 上给出 −7.0% 和 +0.7%，
M=2 上给出 −11.5% 和 −5.9%。**三轮的方向一致（M ≥ 2 不慢），
但幅度在 0–11% 之间摆，和 §2 的漂移同一量级。**
诚实的说法是：**端到端 M ≥ 2 的收益在 0–7%，不稳定；M=1 是明确的负收益。**

拆开看，dispatch A 上的收益是真的、也是稳定的：M=6 的 A 在三轮里分别是
137.5 → 158.8、129.6 → 168.1、131.3 → 152.1 GB/s（**+15% … +30%**），
每一轮它都是 M=6 全场最好的 A。

**但 §3.6 的结论仍然不成立**：M=6 的 A+B 只有 **137–147 GB/s = 上限的
63–67%**，离 §15 P2 的 80% 还差得远。原因很直接——瓶颈换到了 dispatch B
（M=6 时 99–108 GB/s），而 **B 的激活是 `h` 不是 x**，这条路对它无能为力。
§3.6 写的「这是唯一能让 M=6 摸到 80% 线的设计」，实测是错的。

`RowsPerLane` 这次确实能往上开一格——int8 路径不持有 `float d[R][8]`，
寄存器压力只有 fp32 路径的一半，M=1 时 `L16 R4` 是最好的 int8 变体
（0.689–0.700 ms）——但 R=8 依然塌方（M=6 的 A 只有 46.2 GB/s），
...
1. **过不了 design §12 的 5e-3。** (0, 0) 的 2.9e-3 过得去，(39, 383) 的
   8.9e-3 过不去，两者差 3 倍。一个判据不能靠挑 expert 来满足，
   而两个 expert 已经足以说明这个量随 expert 的权重分布走。
...
它唯一说得通的用法是 design §10 的**投机验证批**：那里 M ≥ 2（收益全在那边），
而且接受检查本来就要拿 draft 和 target 对比，对 MoE 输出的近似有容忍度——
但**即使在那里也要先由 L2 决定 8.9e-3 能不能接受**，§10.3 的 0–7% 不值得
拿一个没被批准的数值代价去换。**绝不要把它设成 decode 的默认**：
M=1 上它本来就是负收益（存档那一轮 +15.9%，上一轮 +1.2%）。
...
| 热（存档那一轮） | 1 | 0.734（A 0.448 + B 0.275） | 0.748（**+0.014**, +1.9%） | 0.693（**−0.041**, −5.6%） |
| 热（存档那一轮） | 6 | 1.032（A 0.673 + B 0.367） | 1.079（**+0.047**, +4.5%） | 1.058（**+0.026**, +2.5%） |

（单位 ms/层。热的那两行绝对值高 10–20%——这正是 §2 的漂移——
但**差额**在冷热两种状态下是一致的，因为现在是轮转测量的。
`A + B` 与 `whole` 在每一行都对得上，这是这一节以前没有的自检。）

**八行里没有一行超过 0.05 ms/层，`split A + 一次 B` 全部 ≤ 0.026 ms/层
（有一行甚至是负的 −0.041，那就是漂移在往另一个方向走）。**
...
- `MoeSpec::h_quant` 多一个取值 **3**：`h` 的 fp8 量化由 A 与 B 之间的第三个
  ...
  在它自己的默认形状 `L32 R1` 上那是 **+45.5%**（§9.4.1）；
  改成 3 是一行的事，代价降到约 **+3%**（M=1 四轮的中位数，§9.4）。
...
投机验证（M=6）      : L16 R2, sg32, dec0, h=fp16, XMode=4, x_mode_b=3, HQuant=3
                       —— 若接受检查容得下 x 的近似，XMode=6 的 dispatch A
                          稳定快 15-30%，端到端 0-7% 且不稳定（§10.3）
...
带 validation layer 跑一遍（八个用例与 `--only "h fp8"` / `--only "int8 x"`
两节当前都是干净的——v0.2 的两个新 dispatch 都在里面——
