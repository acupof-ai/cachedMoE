...
| k | M=k+1 | u(M) | E[tokens](k) | expert 读 / 发出 token | 相对无投机（6） |
|---:|---:|---:|---:|---:|---:|
...
1. 最好的一格是 **k = 2，省 ~12% 的 expert 读**；k = 5 仍然是**亏的**（1.08×）。
   §2.2 的"按小 k 算"这条结论，并集之后、用实测 u(M) 之后，**依然成立**。
2. **而 12% 还是个上界，真实收益只会更小**：上表拿"每个 expert 读一遍"当字节，
   但无投机的顺序 decode 是**带热缓存**的（`docs/p4_hitrate.md` 实测 hit 0.837），
   而缓存本来就把"同一个 expert 在相邻几个 token 里被用第二次"变成命中——
   那正是并集省下的那一部分。换句话说，**并集省的和 LRU 已经在省的是同一笔钱**，
   两者重叠多少没量。要把它变成可信的数字，得用 `tools/cache_sim.py` 跑
   "M 个连续位置当一批" vs "逐 token"的 miss 数，而不是 unique 数。
3. 把 0.88–0.90 的字节和 19–42 ms 的 `T_draft` 一起代进去，k=2 贪心大约是
   2.5 个 token / (2.5 × 0.881 × 274 + ~30) ≈ **253 ms/token**，对 274 ms **约 8%**——
   而且这 8% 还要被第 2 条打折。
4. **go / no-go：NO-GO，不要把投机设成默认。** 模型给的 ×1.18–1.32 在这台机器上
   没有出现，原因不是接受率（接受率就是 §12.2 那些数，实测过了），
   而是**代价模型不对**：×1.18–1.32 假设 verify 的代价随 M 摊薄，
   而在一个 NVMe 受限的 MoE 上，代价随 **expert 并集**长，并集在 M 上是 0.82 → 0.60 的
   次线性，**不是 1/M**。
5. 值得保留的是**开关**（`--spec greedy --spec-k 2`）和上面这张表：
   一旦 R1 的 cache/hit 把 MB/token 压下来，kernel 那一半（verify 已经比 6 次 M=1 便宜
   0.62×）就会浮出水面，那时同一张表要重算。

### 6.6 下一步，按价值排序

1. ~~量真实的 u(M)~~ **做了**（`tools/route_union.py`，§6.5）：u(6) = 21.6 而不是外插的 26，
   但没有翻盘。**接着要量的是 miss 而不是 unique**（§6.5 结论 2）：
   用 `tools/cache_sim.py` 把"M 个连续位置当一批取"和"逐 token 取"在同一条 trace、
   同一个容量下跑一遍，比较 miss 字节。这才是并集相对**热缓存**的真实收益，
   而上面那 12% 只是它的上界。半天，不需要循环，且它决定 go / no-go 还值不值得再问。
