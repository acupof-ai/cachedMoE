...
| **第二个读源落地（Track D2）** | 同一份 checkpoint 在一块 USB SSD（E:）上的逐字节副本，`--mirror DIR` / `DEEPMOE_MODEL_MIRRORS` 打开，**默认关**。路由是**加权最小在飞字节**（权重 = 启动时 1 s / 4 MiB / QD 8 的随机读探针），只路由 P0 与 P3。拷贝 **510 GB / 529 s = 964 MB/s**，94 个文件长度全对、5 个抽样 SHA-256 全对。聚合随机读（同一个 6.8 GB shard，4 MiB，QD 16）：D: 单盘 **3.453** → D:+E: **4.555 GB/s（+31%）**，分流 **78 : 22**，**D: 的份额一点没掉**（3.53 vs 3.47）。⚠️ **端到端仍然没测成，两次都倒在同一块盘上**（§3 的 58 与 **59**）。**2026-09-19 下午复测（Track D3）**：E: 物理重插之后 `ls` / `nvme_bench`（**0.962 GB/s**）**全过**，引擎带 `--mirror` 仍然死在初始化——`pinned load of 'norm.weight': overlapped read failed (**win32 1117 = ERROR_IO_DEVICE**)`，**和上午同一个 tensor**。**A/B 表不存在，因为 B 臂起不来**；off 臂今天是 **4.5693 tok/s / stall 113.5 ms / hit 0.8957**（5,000 槽）。**轻 I/O 恢复 ≠ 盘可用** | §3 的 58 / **59**；`p4_dual_source.md` §4 / **§7** |
| **第二个读源：第三次也没测成（Track D4）** | **DX 的验收门 PASS，引擎照样死。** E: 换了 USB 口 + `chkdsk /spotfix` 之后，`dx_probe` 48 句柄 × QD 24 **两条 300 秒臂各 1.04 GB/s、零错误**，句柄开销 **17 s → 158 ms**；引擎第一个 `--mirror` cell 仍然 **win32 1117 / `norm.weight` / off 1323827200**（第四次复现），摘源时的码是 **win32 433 = ERROR_NO_SUCH_DEVICE**。off 臂基线 **4.5607 tok/s / stall 113.0 ms / hit 0.8957**（auto = 5,000 槽）。DX §5.2 的三条闸（启动 pinned 试读 / 失败摘镜像 / 运行期连续 3 次 I/O 错误动态摘源）**全部落地并进 main**，默认仍然**关** | `p4_dual_source.md` §8；§3 的 60 |
