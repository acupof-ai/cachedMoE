...
//
// --copy additionally times the two ways a staging fix would move one expert
// (18.8 MB) out of pinned host memory into path A memory: a CPU memcpy and a
// vkCmdCopyBuffer.
#include <algorithm>
#include <atomic>
#include <chrono>
...
void measure_copies(gpu::MemoryAllocator* alloc_a) {
    ...
    auto dstv = make_vulkan(*alloc_a, MemoryPath::DeviceLocalHostVisible, 1, n);
...
int usage() {
    std::puts(
        ...
        "  --dst LIST      ram,pinned,patha,pathb (default all four)\n"
        "  --reads N       requests per point (default 64)\n"
        "  --copy          also time a pinned -> path A staging copy of one expert\n"
...
int main(int argc, char** argv) {
    ...
    const bool wants_gpu = std::any_of(o.dst.begin(), o.dst.end(),
                                       [](const std::string& s) { return s == "patha" || s == "pathb"; });
    if (wants_gpu || o.copy) {
    ...
    std::puts("dst        qd      GB/s   mean_ms    max_ms   QD@issue  submit_us  note");
    std::puts("-----------------------------------------------------------------------------------------");

    std::vector<Point> results;
    for (const std::string& k : o.dst) {
        for (uint32_t qd : o.qd) {
            auto p = measure(o.file, k, pa, pb, o.req_kb, o.chunk_kb, qd, o.reads);
            if (!p) {
                std::puts(std::format("{:<10} {:>3}   {}", k, qd, p.error().str()).c_str());
                continue;
            }
            results.push_back(*p);
            std::puts(std::format("{:<10} {:>3} {:>9.3f} {:>9.3f} {:>9.3f} {:>10.2f} {:>10.1f}  {}",
                                  p->dst, p->qd, p->gbps, p->mean_ms, p->max_ms,
                                  p->qd_at_issue, p->submit_us, p->note).c_str());
        }
    }
    ...
    if (!o.csv.empty()) {
        if (std::FILE* c = std::fopen(o.csv.c_str(), "wb")) {
            std::fputs("dst,req_kb,chunk_kb,qd,bytes,seconds,gbps,mean_ms,max_ms,qd_at_issue,note\n", c);
            for (const Point& p : results)
                std::fputs(std::format("{},{},{},{},{},{:.6f},{:.4f},{:.4f},{:.4f},{:.2f},{}\n",
                                       p.dst, p.req_kb, p.chunk_kb, p.qd, p.bytes, p.seconds,
                                       p.gbps, p.mean_ms, p.max_ms, p.qd_at_issue, p.note).c_str(), c);
            std::fclose(c);
            std::puts(std::format("\nwrote {}", o.csv).c_str());
        }
    }
